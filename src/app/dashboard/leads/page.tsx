...
export default function LeadsPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return res.json().then((d) => d.leads as Lead[]);
    },
  });
  ...
  const exportCSV = () => {
    const csv = [
      "Name,Email,Phone,Company,Project Type,Budget,Timeline,Status,Date",
      ...leads.map((l) =>
        [
          l.fullName,
          l.email,
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="flex flex-wrap gap-1.5">
          {["ALL", "NEW", "CONTACTED", "PROPOSAL_SENT", "NEGOTIATION", "WON", "LOST"].map((s) => (
            <button
              key={s}
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12 text-sm text-zinc-500">No leads found.</div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-zinc-200 bg-zinc-50 dark:border-zinc-800 dark:bg-zinc-900">
                ...
                <th className="px-4 py-3 text-left font-medium text-muted-foreground dark:text-zinc-400">Contact</th>
                <th className="px-4 py-3 text-left font-medium text-muted-foreground dark:text-zinc-400">Project</th>
            ...
            <tbody>
              {filtered.map((lead) => (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <p className="text-zinc-700 dark:text-muted-foreground">{lead.projectType}</p>
                    {lead.budget && (
                      <p className="text-xs text-zinc-500">{lead.budget}</p>
                    )}
                  </td>
                  <td className="px-4 py-3">
                    <span className={cn("inline-flex rounded-md px-2 py-0.5 text-xs font-medium", statusColors[lead.status])}>
        ...
        return (
          <div className="rounded-xl border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-900">
            <h3 className="font-semibold text-zinc-900 dark:text-white mb-4">
              {lead.fullName} — Details
            </h3>
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-2 text-sm">
                <p><span className="font-medium text-zinc-500">Email:</span> {lead.email}</p>
                <p><span className="font-medium text-zinc-500">Phone:</span> {lead.phone || "N/A"}</p>
                <p><span className="font-medium text-zinc-500">Company:</span> {lead.company || "N/A"}</p>
                <p><span className="font-medium text-zinc-500">Project:</span> {lead.projectType}</p>
              </div>
              <div className="space-y-2 text-sm">
                <p><span className="font-medium text-zinc-500">Budget:</span> {lead.budget || "N/A"}</p>
                <p><span className="font-medium text-zinc-500">Timeline:</span> {lead.timeline || "N/A"}</p>
                <p><span className="font-medium text-zinc-500">Submitted:</span> {new Date(lead.createdAt).toLocaleDateString()}</p>
                <div>
            ...
            <div className="mt-4 rounded-xl bg-zinc-50 p-4 dark:bg-zinc-800/50">
              <h4 className="text-sm font-medium text-zinc-700 dark:text-muted-foreground mb-2">Project Details</h4>
              <p className="text-sm text-muted-foreground dark:text-zinc-400 leading-relaxed">{lead.projectDetails}</p>
            </div>
          </div>
        );
      })()}
