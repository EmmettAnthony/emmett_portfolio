...
const defaultForm: CategoryForm = { name: "", slug: "", description: "", icon: "", order: 0, active: true };
...
export default function SupportCategoriesPage() {
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data: payload }: { id: string; data: Partial<CategoryForm> }) => {
      const res = await fetch(`/api/support/categories/${id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
      });
  ...
  const updateField = (key: keyof CategoryForm, value: string | number | boolean) => {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-zinc-500">{categories.length} total categories</p>
        </div>
              ...
              <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
                {categories.map((cat) => (
                  <tr key={cat.id} className="transition-colors hover:bg-zinc-50 dark:hover:bg-zinc-800/50">
                    ...
                    <td className="px-4 py-3 text-sm font-mono text-zinc-500 dark:text-zinc-400">{cat.slug}</td>
                    <td className="px-4 py-3 text-sm text-zinc-500 max-w-[200px] truncate">{cat.description ?? "—"}</td>
                    <td className="px-4 py-3 text-sm text-zinc-500">{cat.order}</td>
                    <td className="px-4 py-3">
                      <button
