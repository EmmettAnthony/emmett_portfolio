// ──────────────────────────────────────────────────────────────────────────────
// Notification Bus — CRM Proposal End-to-End Integration Test
// ──────────────────────────────────────────────────────────────────────────────
// Tests the full pipeline from PUT /api/dashboard/crm/proposals/[id] →
// notifyCrmProposalApproved → sendNotification → notification-bus emit.
// ──────────────────────────────────────────────────────────────────────────────
...
const mockProposalFindUnique = vi.hoisted(() => vi.fn());
const mockProposalUpdate = vi.hoisted(() => vi.fn());
...
beforeEach(() => {
  ...
  mockProposalFindUnique.mockResolvedValue(BASE_PROPOSAL);
  mockProposalUpdate.mockResolvedValue(BASE_PROPOSAL);
...
describe("Notification Bus E2E: PUT CRM proposal APPROVED → bus", () => {
  ...
  it("delivers a bus event when proposal status changes to APPROVED via PUT handler", async () => {
    const { onNotification } = await import("@/lib/notifications/notification-bus");
    const busEvents: unknown[] = [];
    const unsubscribe = onNotification((event) => busEvents.push(event));
    busCleanups.push(unsubscribe);

    mockProposalUpdate.mockResolvedValue(APPROVED_PROPOSAL);

    const { PUT } = await import("@/app/api/dashboard/crm/proposals/[id]/route");

    const request = new Request("http://localhost:3000/api/dashboard/crm/proposals/prop-e2e-1", {
      method: "PUT",
...
request as any, {
      params: Promise.resolve({ id: "prop-e2e-1" }),
    });
    ...
    expect(busEvent.eventKey).toBe("crm.proposal.approved");
    expect(busEvent.notification.source).toBe("crm");
    expect(busEvent.notification.title).toBe("Proposal Approved: Web Development Proposal");
    expect(busEvent.notification.message).toContain("Bob Smith");
    expect(busEvent.notification.link).toBe("/dashboard/crm/proposals/prop-e2e-1");
  ...
  it("does NOT deliver a bus event when status is unchanged", async () => {
    ...
    mockProposalFindUnique.mockResolvedValue({ ...BASE_PROPOSAL, status: "APPROVED" });
    mockProposalUpdate.mockResolvedValue({ ...BASE_PROPOSAL, status: "APPROVED" });
...
request as any, { params: Promise.resolve({ id: "prop-e2e-1" }) });

    expect(busEvents).toHaveLength(0);
  });

  it("returns 404 and does NOT deliver bus event when proposal not found", async () => {
    const { onNotification } = await import("@/lib/notifications/notification-bus");
    const busEvents: unknown[] = [];
    const unsubscribe = onNotification((event) => busEvents.push(event));
    busCleanups.push(unsubscribe);
